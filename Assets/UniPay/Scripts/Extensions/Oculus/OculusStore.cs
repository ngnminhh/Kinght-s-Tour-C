...
namespace UniPay
{
    ...
    {      
        ...
        public virtual void Purchase(ProductDefinition definition, string developerPayload)
        {
            ...
            #if UNITY_EDITOR
                IAPManager.GetInstance().CompletePurchase(definition.id);
            #else
        ...
        public void OnPurchaseSucceeded(Message<Purchase> result)
        {
            ...
            //allow for multiple purchases requiring unique transaction IDs
            //Oculus GetPurchase().ID returns garbage in both test and real transactions
            //so we use the purchase GrantTime converted to UNIX timestamp
            DateTimeOffset timeOffset = new DateTimeOffset(result.Data.GrantTime);

            UnifiedReceipt receipt = new UnifiedReceipt()
            {
                Payload = string.Empty,
                Store = IAPPlatform.OculusStore.ToString(),
                TransactionID = timeOffset.ToUnixTimeMilliseconds().ToString()
            };

            callback.OnPurchaseSucceeded(result.GetPurchase().Sku, JsonUtility.ToJson(receipt), receipt.TransactionID);
        }
